...
class TestHyperparameter(unittest.TestCase):
    ...
    def test_float(self):
        ...
        self.assertEqual(hyp.rang, [1.5,3.2])
        transformed = hyp.fit_transform(np.array([2,2.4,3.1]), np.array([0.5,0.6,0.1]))
    ...
    def test_float_exp(self):
        ...
        self.assertEqual(hyp.rang, [-3.0, 2.0])
        transformed = hyp.fit_transform(np.array([0.01, 1,10]), np.array([-2.0,0.0,1.0]))
        numpy.testing.assert_array_equal(transformed, np.array([-2.0, 0.0, 1.0]))
    ...
    def test_int_cat(self):
        ...
        transformed = hyp.fit_transform(np.array([10, 10000]), np.array([1,2]))
        numpy.testing.assert_array_equal(transformed, np.array([1,2]))
        inverse_transform = hyp.inverse_transform([3, 0, 1, 2])
        numpy.testing.assert_array_equal(inverse_transform, np.array([10000, 10, 10, 10000]))

    def test_float_cat(self):
        hyp = HyperParameter(ParamTypes.FLOAT_CAT, [0.1, 0.6, 0.5])
        transformed = hyp.fit_transform(np.array([0.1, 0.6, 0.1, 0.6]), np.array([1,2, 3,4]))
        numpy.testing.assert_array_equal(transformed, np.array([2.0,3.0, 2.0,3.0]))
        self.assertEqual(hyp.rang, [0.0,3.0])
        inverse_transform = hyp.inverse_transform([3, 0, 1, 5, 2.5])
        numpy.testing.assert_array_equal(inverse_transform, np.array([0.6, 0.5, 0.1, 0.6, 0.6]))

    def test_bool(self):
        hyp = HyperParameter(ParamTypes.BOOL, [True, False])
        transformed = hyp.fit_transform(np.array([True, False]), np.array([0.5, 0.7]))
        numpy.testing.assert_array_equal(transformed, np.array([0.5, 0.7]))
        self.assertEqual(hyp.rang, [0.5,0.7])
        inverse_transform = hyp.inverse_transform([0.7, 0.6, 0.5])
        numpy.testing.assert_array_equal(inverse_transform, np.array([False, False, True]))

    def test_string(self):
        hyp = HyperParameter(ParamTypes.STRING, ['a', 'b', 'c'])
        transformed = hyp.fit_transform(np.array(['a', 'b', 'c']), np.array([2,1,3]))
        self.assertEqual(hyp.rang, [1.0, 3.0])
        numpy.testing.assert_array_equal(transformed, np.array([2.0, 1.0, 3.0]))
        inverse_transform = hyp.inverse_transform([3])
