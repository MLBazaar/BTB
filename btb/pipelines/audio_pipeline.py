...
class AudioPipeline(DmPipeline):
    """Pipeline class for audio datasets
    ...
    def _load_train_data(self, d3mds):
        audio_dir = d3mds.dataset.get_audio_path()
        #TODO fix audio_file
        audio_names = d3mds.get_train_data()['audio_file'].values.T
        labels = d3mds.get_train_targets()[:, 0]
        return audio_dir, audio_names, labels

    def _train_data_generator(self, audio_dir, audio_names, labels, batch_size=None):
        if batch_size is None:
            batch_size = self.batch_size
        batch_start_idx = 0
        for audio_batch in self._audio_data_generator(audio_dir, audio_names, batch_size):
            batch_end_idx = batch_start_idx + len(audio_batch)
    ...
    def _text_data_generator(self, audio_dir, audio_filenames, batch_size=None, terminate = False):
        if batch_size is None:
            batch_size = self.batch_size
        while True:
            audio_batch = []
    ...
    def _build_model(self, d3mds, cv_scoring = None, cv = None):
        audio_dir, audio_names, labels = self._load_train_data(d3mds)
        data = self._text_data_generator(audio_dir, audio_names)
